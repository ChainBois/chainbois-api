...
**Answer:**
- **Backend:** https://github.com/ChainBois/chainbois-api
- **Frontend:** https://github.com/ChainBois/chainbois-true
...
- 9 automated cron jobs (score sync, tournament lifecycle, tokenomics sweep, wallet health, platform audit, new user sync, purchase failsafe, failed payout retry, trait airdrop)
...
- Backend writes player state (hasNFT, level, weapons, characters) → game reads
...
│  Auth │ Training │ Battleground │ Armory │ Points │ Inventory    │
│  Game Sync │ Leaderboard │ Metadata │ Airdrops │ Claims │ Metrics│
│                                                                   │
│  9 Cron Jobs │ 5 Encrypted Wallets │ Anti-Cheat │ Failsafe      │
└──────────────────────┬──────────────────────────────────────────┘
                       │
         ┌─────────────┼─────────────┐
         ▼             ▼             ▼
...
- Game reads player state from Firebase RTDB: `hasNFT`, `level`, `weapons[]`, `characters[]`
...
**Backend API (Express.js — 50+ Endpoints, 9 Cron Jobs):**

*Controller Modules:*
- **Auth Controller:** Login/logout by wallet address, user creation, Firebase UID linking
- **Game Controller:** Game download (PC/APK), asset verification, avatar selection
- **Training Controller:** Level-up (pay AVAX → increment on-chain level), rank calculation, badge overlay generation via Cloudinary URL transforms
...
2. **syncNewUsersJob (daily midnight):** Detects unregistered Firebase UIDs → tracks Web2 players for metrics
...
- **ChainBoisNFT (ERC-721):** 4,032 total (4,000 public + 32 reserved, 50 on testnet). On-chain level storage (0-7), EIP-4906 metadata update events, dynamic metadata via API with Cloudinary badge overlays.
...
**On-chain (Avalanche Fuji C-Chain):**
- $BATTLE token balances and transfers
- $BATTLE burns (permanent supply reduction)
- ChainBoi NFT ownership and on-chain level (0-7)
- Weapon NFT ownership and on-chain weapon names
- All purchase and prize distribution transactions (verifiable on Snowtrace)

**Off-chain (MongoDB + Firebase):**
- User profiles, UIDs, login history
- Game scores, match results, anti-cheat threat scores
- Tournament definitions, schedules, prize configurations
- Leaderboard rankings and historical snapshots
- Points balances (pre-conversion, off-chain accumulation)
- Platform metrics and audit logs
- NFT metadata JSON (stored on IPFS, referenced on-chain via tokenURI)
- Dynamic badge overlay configurations
...
**Step 1 — Download the Game:** Players download ChainBois from the frontend at [chainbois-true.vercel.app](https://chainbois-true.vercel.app) — available as a PC build or Android APK.

**Step 2 — Create an In-Game Account:** On first launch, the game prompts the player to create an account with a username and email. This registers them in Firebase and they can immediately start playing — no wallet required.

**Step 3 — Connect a Wallet (Optional):** When ready to go on-chain, APK users tap a button inside the game that redirects them to the website's `/access-request` page. PC users navigate there manually. On this page they connect a MetaMask or Core wallet on the Avalanche Fuji Testnet, linking their wallet address to their game account.

**Step 4 — Claim the Starter Pack:** New players visit the [Testnet Faucet](https://chainbois-testnet-faucet.vercel.app) and paste their wallet address. One click delivers: **2 ChainBoi NFTs** (unlocking playable characters), **8 weapon NFTs** (one from each category), and **1,000 $BATTLE tokens**. The platform pays all gas — zero cost to the player. Max 1 claim per wallet.

**Step 5 — Backend Verification & Sync:** The backend verifies the player's on-chain assets (NFTs, weapons, $BATTLE balance), writes the results to Firebase, and the game reads this data to unlock characters, weapons, and features automatically.

### Core Gameplay

**Step 6 — Play Across 7 Game Modes:** Players compete across Frontline, Team Deathmatch, Kill Confirmed, Gun Fight, Battle Royale, Capture the Flag, and Search and Destroy — earning points based on performance.

**Step 7 — Score Syncing with Anti-Cheat:** The game writes scores to Firebase. Every 5 minutes, the backend syncs scores to MongoDB with anti-cheat validation — plausibility checks, velocity analysis, threat scoring, and automatic bans for flagged accounts.

### Progression & Economy

**Step 8 — Training Room (Level Up Your NFT):** Players level up their ChainBoi NFT from Level 0 through Level 7 (army ranks: Private → Corporal → Sergeant → Captain → Major → Colonel → Major General → Field Marshal) by paying AVAX. Each level unlocks 4 new playable characters and additional weapons. The level is recorded on-chain, and the NFT's metadata updates dynamically with new rank, stats, and Cloudinary badge overlays — visible immediately on Snowtrace and marketplaces.

**Step 9 — Armory (Buy Weapons with $BATTLE):** The Armory offers 30 weapons across 8 categories purchasable with $BATTLE tokens. Players can also purchase additional ChainBoi NFTs with AVAX. Every purchase includes a failsafe mechanism — if the on-chain transfer fails, the transaction is rolled back and the player is refunded. A portion of spent $BATTLE is automatically burned, creating deflationary pressure.

**Step 10 — Points → $BATTLE Conversion:** Players convert earned in-game points to $BATTLE tokens at a dynamic exchange rate. The rate adjusts automatically based on the health of the rewards wallet across five tiers (ABUNDANT → HEALTHY → MODERATE → SCARCE → CRITICAL), ensuring long-term sustainability.

**Step 11 — Tournaments (Compete for Prizes):** The Battleground features 7 tournament tiers (one per player level) with 5-day cycles. When a tournament ends, prizes are automatically distributed — AVAX to 1st/2nd place, $BATTLE to 3rd. No manual claiming required. Discord webhook notifications announce winners.

**Step 12 — Weekly Airdrops:** Every Wednesday, the system runs a trait-based airdrop — $BATTLE tokens are distributed to NFT holders based on specific traits and rarity. Higher-level NFTs with rarer traits receive larger allocations.

### Viewing & Tracking

**Step 13 — Inventory:** Players view all owned ChainBoi NFTs (with current level, rank, and badge artwork), weapons, $BATTLE balance, and full transaction history in one place.

**Step 14 — Leaderboard:** Global rankings display player performance across multiple time periods (30 minutes, 1 hour, 24 hours, week, month, all-time).

**Step 15 — Platform Metrics:** Track total players (Web2 vs Web3), NFTs minted, tournaments completed, and $BATTLE in circulation.

### Web2 Accessibility

**Step 16 — Web2 Players Welcome:** Players without a wallet can download the game, create an account, and play immediately. Their scores are tracked and ranked. When they connect a wallet later, their existing progress carries over — they gain NFT ownership, $BATTLE earning, tournament prizes, airdrops, and dynamic NFT metadata reflecting their achievements.
...
- Wallet connection and user authentication (Thirdweb + Firebase Auth)
- Game download (PC + APK) and Firebase game bridge (play → earn points)
...
- $BATTLE → AVAX cashout (DEX integration)

---

**6. Question:** Walkthrough Video
**Answer:** [TO BE RECORDED — max 5 minutes demonstrating: download game → create account → connect wallet → claim starter pack → play → level up → buy weapon → tournament leaderboard]
...
- **Backend API (Health Check):** https://test-2.ghettopigeon.com/api/v1/health
- **Testnet Faucet (Claim Page):** https://chainbois-testnet-faucet.vercel.app
- **Backend GitHub:** https://github.com/ChainBois/chainbois-api
- **Frontend GitHub:** https://github.com/ChainBois/chainbois-true
...
- **Training Room** — Level up ChainBoi NFTs through 8 army ranks (Private → Field Marshal), with on-chain level updates and dynamic badge overlays. Each level unlocks 4 new playable characters.
...
ChainBois is designed so anyone can play immediately, whether they have a crypto wallet or not. We built a dual-track onboarding system that lets Web2 players jump straight into the game while giving Web3 players full ownership of their in-game assets.

**Web3 Players (Have a Wallet):**

1. **Download the game** — grab the PC build from the ChainBois frontend at [chainbois-true.vercel.app](https://chainbois-true.vercel.app) or install the Android APK directly.
2. **Create an account in-game** — standard sign-up with username and email inside the Unity client. This registers the player in Firebase.
3. **Connect your wallet** — the APK has a built-in button that redirects to the ChainBois website for wallet connection. PC players navigate to `/access-request` manually. The website uses Thirdweb's wallet connection flow — players connect MetaMask or Core Wallet on the Avalanche Fuji Testnet.
4. **Claim your starter pack** — visit [chainbois-testnet-faucet.vercel.app](https://chainbois-testnet-faucet.vercel.app) and paste your wallet address. One click delivers everything: **2 ChainBoi NFTs** (unlocking playable characters), **8 weapon NFTs** (one from each category: Assault, SMG, LMG, Marksman, Handgun, Launcher, Shotgun, Melee), and **1,000 $BATTLE tokens**. The player pays zero gas — the platform covers all transaction fees. The entire claim executes 11 on-chain transactions with a real-time progress indicator showing each step. Each wallet can only claim once, enforced by a database unique index and an in-memory lock to prevent double-claims.
5. **Backend verifies on-chain ownership** — the backend reads the blockchain to confirm which NFTs and tokens the player actually holds, then writes their verified assets (characters, weapons, level, rank) to Firebase Realtime Database.
6. **Game reads Firebase automatically** — the Unity client polls Firebase and unlocks the corresponding characters and weapons. No game restart needed. Game developers did not need to change any code for this — the Firebase bridge handles everything.
7. **Play across 7 game modes** — Frontline, Team Deathmatch, Kill Confirmed, Gun Fight, Battle Royale, Capture the Flag, and Search and Destroy. Earn points based on performance.
8. **Scores sync to backend every 5 minutes** — a cron job polls Firebase for new scores, validates them with anti-cheat checks (score plausibility, velocity limits, threat scoring), and syncs verified scores to MongoDB.
9. **Access the full platform** — Training Room (level up NFTs, unlock new characters and army ranks), Armory (buy weapons and NFTs), Tournaments (compete for prizes), Points conversion ($BATTLE), Inventory (view all assets and transaction history), Leaderboard (track rankings).

**Web2 Players (No Wallet):**

Web2 players face zero friction. Download the game and play immediately — no wallet, no blockchain knowledge, no setup required. When a new player creates an account and starts submitting scores, the backend detects their Firebase UID as unregistered (no linked wallet address) and automatically tracks them as a Web2 player. All their scores, progress, and game data are saved in both Firebase and MongoDB. They appear on leaderboards and can compete in every game mode. The daily sync job counts Web2 vs Web3 players for platform metrics.

**Web2 → Web3 Upgrade — What Players Gain:**

When a Web2 player connects a wallet, they unlock the full blockchain-powered experience:

- **Own your characters as NFTs** — ChainBoi NFTs are real on-chain assets, tradeable and sellable on any Avalanche NFT marketplace (Joepegs, etc.)
- **Own your weapons as NFTs** — each weapon is an ERC-721 token with real ownership and transferability
- **Earn $BATTLE tokens from gameplay** — convert accumulated points to $BATTLE at a dynamic exchange rate
- **Compete in tournaments for real prizes** — AVAX and $BATTLE auto-distributed to winners
- **Level up your NFTs** — pay AVAX to increase your ChainBoi's level (0→7), unlocking 4 new characters per level across 8 army ranks (Private → Field Marshal)
- **Receive weekly $BATTLE airdrops** — trait-based distribution every Wednesday, with rarer traits earning higher allocations
- **Dynamic NFT metadata** — your NFT's metadata updates to reflect your actual progress: current level, army rank, games played, points earned, and Cloudinary-generated badge overlay images
- **Full inventory with transaction history** — view every NFT, token balance, and on-chain transaction tied to your wallet

**Key Onboarding Design Decisions:**
- **No gas costs for first interaction** — the platform wallet pays all transaction fees for the starter pack claim
- **One-click claim** — paste address, click one button, receive all assets. No approvals, no signing, no gas estimation
- **Auto-mint if stores are empty** — if pre-minted NFT inventory runs low, the system auto-mints more via the deployer wallet
- **Firebase bridge** — game developers did not need to modify any game code to support blockchain features
- **Max 1 claim per wallet** — enforced at database level with unique index + in-memory concurrency lock
...
- **9 cron jobs running continuously** in production — score sync (5min), leaderboard, tournament lifecycle, tokenomics sweep (6h), wallet health (hourly), platform audit (daily), new user sync (daily), purchase failsafe (5min), failed payout retry (6h).
