...
const levelUp = catchAsync(async (req, res, next) => {
  ...
  await ChainboiNft.findOneAndUpdate(
    ...
    {
      level: newLevel,
      badge: (RANK_NAMES[newLevel] || "trainee").toLowerCase().replace(/ /g, "_"),
  ...
  res.status(200).json({
    ...
    data: {
      ...
      newLevel,
      rank: RANK_NAMES[newLevel] || "Trainee",
      cost,
    ...
    if (parsedLevel >= MAX_LEVEL) {
      return res.status(200).json({
        ...
        data: {
          ...
          currency: "AVAX",
          isMaxLevel: true,
          rank: RANK_NAMES[parsedLevel] || "Trainee",
          nextRank: null,
        },
      });
    }
