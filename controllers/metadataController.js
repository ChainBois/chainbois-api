...
const { RANK_NAMES, DYNAMIC_TRAIT_TYPES } = require("../config/constants");
...
const getTokenMetadata = catchAsync(async (req, res, next) => {
  ...
  if (nft && nft.traits && nft.traits.length > 0) {
    baseTraits = nft.traits
      .filter((t) => !DYNAMIC_TRAIT_TYPES.has(t.trait_type))
    ...
    try {
      ...
      baseTraits = (staticData.attributes || []).filter((a) => !DYNAMIC_TRAIT_TYPES.has(a.trait_type));
  ...
  // Prefer IPFS badge image from MongoDB (pinned during level-up), fall back to Cloudinary overlay
  const ipfsImage = nft && nft.imageUri && nft.imageUri.startsWith("ipfs://") && level > 0 ? nft.imageUri : null;
  const cloudinaryImage = getBadgeOverlayUrl(tokenId, level);
  const fallbackImage = `ipfs://bafybeifd4wjgbvnpf7kmcrkjxp7i4ipz3w2aag3elgfj6v364y2meq6ep4/${tokenId}.png`;
  const imageUrl = ipfsImage || cloudinaryImage || fallbackImage;
