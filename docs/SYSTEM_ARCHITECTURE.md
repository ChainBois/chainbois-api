...
The faucet features **wallet connect with auto chain switching** — users click Connect Wallet, choose from detected wallets (EIP-6963 multi-wallet discovery with legacy fallback), and the page automatically prompts their wallet to add and switch to Avalanche Fuji via EIP-3085/EIP-3326. Wallet connection is required (no manual address entry). The Claim button is disabled until a wallet is connected. CORS headers are set on both success and error responses via the global error handler, ensuring cross-origin error messages are visible to the faucet. See [Wallet Connect Flow](WALLET_CONNECT_FLOW.md) for details.
